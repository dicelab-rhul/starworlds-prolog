...
public class SWIUtils {
	...
	public static <V> V visitList(Term term, TermVisitor<V> visitor) {
		if (term == null) {
			return null;
		} else if (term.name().equals(EMPTYARRAY)) {
			return visitor.getCollection();
		}
		V v = visitor.visit(term.arg(1));
		Term next = term.arg(2);
		while (!next.name().equals(EMPTYARRAY)) {
			v = visitor.visit(next.arg(1), v);
			next = next.arg(2);
		}
		return v;
	}
